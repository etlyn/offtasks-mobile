...
export const localTasksKey = (owner: string) =>
  `@offtasks/tasks:v1:${encodeURIComponent(owner)}`;
...
export async function readLocalTasks(owner: string): Promise<Task[]> {
  ...
  if (
    ...
    throw new Error(
      'Local tasks could not be read. Stored data has not been changed.',
    );
  }
  return tasks;
}

export function changeLocalTasks(
  owner: string,
...
export const updateLocalTask = (
  ...
  changeLocalTasks(owner, tasks => {
    ...
    return tasks.map(task =>
      task.id === id ? { ...task, ...updates, id, user_id: owner } : task,
    );
  });
