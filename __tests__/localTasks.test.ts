...
test('guest CRUD persists independently and concurrent writes are serialized', async () => {
  const values = {
    ...
    isComplete: false,
    priority: 0,
    target_group: 'upcoming' as const,
  };
  const [first, second] = await Promise.all([
    createLocalTask(GUEST_ID, values),
    createLocalTask(GUEST_ID, values),
  ]);
  expect(await readLocalTasks(GUEST_ID)).toHaveLength(2);
  expect(await readLocalTasks('account')).toEqual([]);
  await updateLocalTask(GUEST_ID, first.id, {
    content: 'Edited',
    isComplete: true,
  });
  expect((await readLocalTasks(GUEST_ID))[0]).toMatchObject({
    content: 'Edited',
    isComplete: true,
  });
